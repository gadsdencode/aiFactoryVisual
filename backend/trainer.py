...
def create_sft_config(config: AppConfig, train_dataset_size: int) -> SFTConfig:
    ...
    Includes safe normalization of hyperparameters, optimizer fallback for
    bitsandbytes-dependent choices, and derivation of max_steps when unset.
    """
    # Normalize/resolve optimizer with fallback when bitsandbytes not available
    chosen_optim = getattr(config.training, 'optim', 'adamw_torch') or 'adamw_torch'
    needs_bnb = ('8bit' in str(chosen_optim)) or ('paged' in str(chosen_optim))
    if needs_bnb:
        try:
    ...
    # Derive total optimizer steps if max_steps is not set
    if max_steps_val <= 0:
        steps_per_epoch = max(1, math.ceil(max(1, train_dataset_size) / max(1, per_device_bs_val)))
        opt_steps_per_epoch = max(1, math.floor(steps_per_epoch / max(1, grad_accum_val)))
        final_max_steps = max(1, opt_steps_per_epoch * max(1, num_train_epochs_val))
    else:
        final_max_steps = max_steps_val
    ...
    # Build SFTConfig (preserve previous non-default args)
    return SFTConfig(
...
def prepare_tokenized_datasets(config: AppConfig, tokenizer, dataset, eval_dataset, emit_log=None):
    """Prepare tokenized datasets using on-disk caching and set proper formats.

    Returns a tuple (tokenized_train, tokenized_eval_or_none).
    """
    # Stable single-thread CPU to avoid Windows deadlocks during heavy preprocessing
    try:
        import os as _os
    ...
    # Resolve tokenizers parallelism from config
    desired_proc = int(getattr(config.training, 'tokenizer_num_proc', None) or 0)
    if desired_proc <= 0:
        import os as _os
        desired_proc = max(1, min(_os.cpu_count() or 1, 8))
    try:
        import os as _os
        _os.environ["TOKENIZERS_PARALLELISM"] = "true" if desired_proc > 1 else "false"
        if emit_log is not None:
            emit_log(f"Tokenization parallelism resolved: num_proc={desired_proc}, TOKENIZERS_PARALLELISM={_os.environ.get('TOKENIZERS_PARALLELISM')}")
    except Exception:
        pass
...
def create_data_collator(tokenizer):
    """Create an efficient data collator with fallback when transformers default is unavailable."""
    try:
...
def build_trainer(model, tokenizer, sft_config: SFTConfig, tokenized_train, tokenized_eval, data_collator, callbacks):
    """Construct the SFTTrainer with compute_metrics only when we have eval data and strategy not 'no'."""
    has_eval = tokenized_eval is not None and getattr(sft_config, 'eval_strategy', None) is not None
    # Our config uses training.evaluation_strategy; SFTConfig stores it as evaluation_strategy
    evaluation_strategy = getattr(sft_config, 'evaluation_strategy', 'no')
    compute_metrics_fn = compute_token_accuracy if (tokenized_eval is not None and evaluation_strategy != 'no') else None
